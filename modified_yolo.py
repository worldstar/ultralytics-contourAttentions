...
class SpatialAttention(nn.Module):
    ...
    def compute_contours(self, x):
        ...
        for img_batch_idx in range(x_np.shape[0]):
            img_feature_map = x_np[img_batch_idx]

            # 1. Convert to single-channel grayscale (0-255)
            img_gray_float = np.mean(img_feature_map, axis=0)
            img_min = img_gray_float.min()
            img_max = img_gray_float.max()
            if img_max == img_min:
                normalized_gray = np.zeros_like(img_gray_float)
            else:
                normalized_gray = (img_gray_float - img_min) / (img_max - img_min)
            gray_uint8 = (normalized_gray * 255).astype(np.uint8)

            # 2. --- NEW CONTOUR LOGIC ---
            # Apply a blur to reduce high-frequency noise
            ...
            # 3. --- CREATE FINAL MASK ---
            # Create an empty mask and draw all found contours on it (filled in)
...
class TripleDDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        box, _cls = torch.cat([di.view(shape[0], self.no, -1) for di in d1], 2).split((self.reg_max * 4, self.nc), 1)
        dist2bbox(self.dfl(box), self.anchors.unsqueeze(0), xywh=True, dim=1) * self.strides
        box2, _cls2 = torch.cat([di.view(shape[0], self.no, -1) for di in d2], 2).split((self.reg_max * 4, self.nc), 1)
